...
void flotarMAX(pmonticulo m, int pos){
    int padre = (pos-1) / 2;
    int temp;
    while(pos > 0 && m->vector[padre] < m->vector[pos]){
        temp = m->vector[padre];
...
void hundirMAX(pmonticulo m, int pos){
    int mayor = pos;
    int hijoIzq = 2 * pos + 1;
    int hijoDer = 2 * pos + 2;

    if (hijoIzq <= m->ultimo && m->vector[hijoIzq] > m->vector[mayor]) {
        mayor = hijoIzq;
    }
    if (hijoDer <= m->ultimo && m->vector[hijoDer] > m->vector[mayor]) {
        mayor = hijoDer;
    }
    if (mayor != pos) {
        int temp = m->vector[pos];
        m->vector[pos] = m->vector[mayor];
        m->vector[mayor] = temp;
        hundirMAX(m, mayor);
    }
}
...
void insertarMonticuloMAX(pmonticulo m, int x) {
    if (m->ultimo == TAM - 1) {
        printf("Montículo lleno\n");
        return;
    }
    m->vector[++(m->ultimo)] = x;
    flotarMAX(m, m->ultimo);
}

void crearMonticuloMAX(pmonticulo m, int v[], int n) {
    int i;
    iniMonticulo(m);
    for (i = 0; i < n; i++) {
        m->vector[i] = v[i];
...
void ordenarPorMonticulosMAX(int v[], int n) {
    struct monticulo m;
    int i;
    crearMonticuloMAX(&m, v, n);
    for (i = n - 1; i >= 0; i--) {
...
void probarKEsimoMayor() {
    ...
    int k = 3;
    printf("El %d-ésimo mayor número es: %d\n", k, k_esimoMAYOR(v, n, k));
}
...
void medicionesMayor(void (*ordenacion)(int[], int), double (*f1)(int), double (*f2)(int), double (*f3)(int)) {
    struct monticulo m;
    int n = 256000;
    int i;
    int k = 10000; // Número de repeticiones para promediar tiempos pequeños
    ...
    for (i = 4000; i <= n; i *= 2) {
        for (int j = 0; j < i; j++) {
            v[j] = i - j; // Vector inicializado
        }
        iniMonticulo(&m);
        t1 = microsegundos();
        ordenarPorMonticulosMAX(v, i);
        t2 = microsegundos();
        t = (t2 - t1);
        if (t < 500) {
            ta = microsegundos();
...
int main(){
    ...
    printf("max.Prueba con orden descendente:\n");
    medicionesMayor(descendente, f1_desc, f2_desc, f3_desc);
    printf("\nMAX.Prueba con orden ascendente:\n");
    medicionesMayor(ascendente, f1_asc, f2_asc, f3_asc);
    printf("\nMAX. Prueba con orden aleatorio:\n");
    medicionesMayor(aleatorio, f1_aleat, f2_aleat, f3_aleat);
